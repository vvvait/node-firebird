...
const
    ...
    isc_action_svc_get_fb_log = isc_action_svc_get_ib_log,
...
const
    ...
    ISOLATION_READ_COMMITTED_NOWAIT = [isc_tpb_version3, isc_tpb_write, isc_tpb_nowait, isc_tpb_read_committed, isc_tpb_no_rec_version],
...
exports.ISOLATION_READ_COMMITTED_NOWAIT = ISOLATION_READ_COMMITTED_NOWAIT;
...
Transaction.prototype.execute = function(query, params, callback, custom) {
    ...
    this.newStatement(query, function(err, statement) {
        ...
        statement.fetchSize = custom?.fetchSize ?? DEFAULT_FETCHSIZE;
        statement.segmentSize = custom?.segmentSize ?? DEFAULT_SEGMENTSIZE;
...
Database.prototype.attachEvent = function(callback) {
    var self = this;
    this.connection.connectRequest(function(err, socket_info) {
        
        if (err) {
            doError(err, callback);
            return;
        }        
        var cnxevent = new EventConnection(self.connection.host, socket_info.port, function(err) {

            if (err) {
                doError(err, callback);
                return;
            }   
            var evt = new FbEventManager(self, cnxevent, self.eventid++, function(err) {
...
FbEventManager.prototype.registerEvent = function(events, callback) {
    ...
    events.forEach(function(event) { self.events[event] = self.events[event] || 0 });
                    ...
                    case op_event:
                        ...
                        while (pos < buf.length) {
                            ...
                            if (prevcount !== 0 && prevcount !== eventcount)
                                lst_event.push({ name: eventname, count: eventcount - prevcount });
                        }
    ...
    self._socket.on('data', function(data) {
        ...
        while (xdr.pos < xdr.buffer.length) {
            ...
            decodeResponse(xdr, cb, self, self._lowercase_keys, function(err, obj) {
                ...
                if (err) {
                    // console.info("decodeResponse", err.message.startsWith('The value of "offset" is out') ? err : err.message);
                    xdr.buffer = xdr.buffer.slice(pos);
                    xdr.pos = 0;
                    self._xdr = xdr;

                    if (err.smallBuf !== true && self.accept.protocolMinimumType === ptype_lazy_send && self._queue.length > 0 && self._queue[0]) {
        ...
        switch (r) {
            ...
            case op_sql_response:
                ...
                if (custom.asObject && !data.fcols) {
                    ...
                    statement.connection.db.emit('columns', data.fcols, output);
                }
...
function parseOpResponse(data, response, cb) {    
    ...
    try {
        ...
        if (e instanceof RangeError) {
            data.pos = initPos;
            e.smallBuf = true;  // чтобы не устанавливать lazy_count
        }        
        if (cb) {
            return cb(e)
        } else {
...
Connection.prototype.startTransaction = function(isolation, callback) {
    ...
    blr.addBytes(isolation || ISOLATION_READ_COMMITTED_NOWAIT);
                        ...
                        case isc_info_sql_type:
                            ...
                            switch (type & ~1) {
                                ...
                                case SQL_BOOLEAN:
                                    ...
                                    break;
                                default:
                                    throw new Error(`Unexpected ${type & ~1}`);
                            }
        ...
        return promiseTransaction.then((transaction) => {
            return new Promise((resolve, reject) => {
                ...
                statement.connection.openBlob(id, transaction, (err, blob) => {
                    ...
                    blob.segmentSize = statement.segmentSize ?? DEFAULT_SEGMENTSIZE;
    
                    const read = () => {
                        const readTimeout = setTimeout(() => {
                            const error = new Error("Read segment timeout");
                            error.column = column;
                            error.row = row;
                            reject(error)
                        }, 5000);
                        statement.connection.getSegment(blob, (err, ret) => {
                            ...
                            infoValue.value = Buffer.concat(segments).toString(DEFAULT_ENCODING);
                            if (singleTransaction) {
...
Connection.prototype.fetch = function(statement, transaction, count, callback) {
    ...
    if (count instanceof Function) {
        callback = count;
        count = statement.fetchSize ?? DEFAULT_FETCHSIZE;
    }
    ...
    msg.addBlr(blr);
    msg.addInt(0); // message number
    msg.addInt(count ?? statement.fetchSize ?? DEFAULT_FETCHSIZE); // fetch count
...
Connection.prototype.fetchAll = function(statement, transaction, callback) {
                ...
                for (let i = 0; i < ret.data.length; i++) {
                    ...
                    if (i === lastIndex) {
                        if (ret.fetched) {
                            ...
                            self.fetch(statement, transaction, statement.fetchSize ?? DEFAULT_FETCHSIZE, loop);
                        }
                    }
                }
            }).catch(callback);
	...
	this.fetch(statement, transaction, statement.fetchSize ?? DEFAULT_FETCHSIZE, loop);
};
...
Connection.prototype.getSegment = function(blob, callback) {
    ...
    msg.addInt(blob.handle);
    msg.addInt(blob.segmentSize ?? DEFAULT_SEGMENTSIZE); // buffer length
    msg.addInt(0); // ???
    this._queueEvent(callback);
};
...
Connection.prototype.connectRequest = function(callback) {
    ...
    function cb(err, ret) {
        ...
        doCallback(socket_info, callback);
    }
    this._queueEvent(cb);
