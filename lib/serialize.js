...
XdrReader.prototype.readArray = function () {
    ...
    if (!len)
        return;
    const reciveLen = (this.buffer.length - 12) - this.pos; // 3 int32 в конце пакета 
    if (len > reciveLen) // оставшихся байт недостаточно для заполнения буфера целиком
        throw new RangeError(`Buffer too small await ${len} exists ${reciveLen}`);;
